...
class ColorReplacer {
    ...
    // Update CSS variables in globals.css or index.css
    async updateCssVariables(rootDir, newColors) {
        // Find CSS file
        const cssFiles = ['src/globals.css', 'src/index.css', 'styles/globals.css', 'styles/index.css', 'src/app/globals.css', 'globals.css', 'index.css'];
        let cssPath = null;
        ...
        if (!cssPath) {
            console.log('⚠️  Could not find a global CSS file to add variables to.');
            return;
        }

        console.log(`🎨 Adding CSS variables to ${cssPath}...`);
        let content = await fs.readFile(cssPath, 'utf8');

        // Generate CSS variables
        const variables = Object.entries(newColors).map(([name, hex]) => {
        ...
        console.log(`✅ Added ${Object.keys(newColors).length} CSS variables`);
    }
    ...
    async updateTailwindConfig(rootDir) {
        ...
        if (this.config.useCssVariables) {
            await this.updateCssVariables(rootDir, newColors);
            // Update configColors to use variables
            Object.keys(colorsForConfig).forEach(name => {
                colorsForConfig[name] = `rgb(var(--${name}) / <alpha-value>)`;
            });
        }
        ...
        try {
            ...
            return this.createTailwindConfig(configPath, colorsForConfig);
        }
        try {
            ...
            if (await this.updateThemeColors(rootDir, configContent, colorsForConfig)) {
                return;
            }
            // Strategy 3: Look for existing theme object and add extend
            if (await this.addExtendToTheme(rootDir, configContent, colorsForConfig)) {
                return;
            }
            ...
            await this.addCompleteTheme(configPath, configContent, colorsForConfig);
        } catch (error) {
            console.error(
    ...
    async updateThemeColors(rootDir, configContent, colorsObj) {
        const configPath = path.join(rootDir, this.config.tailwindConfig);
    ...
    async addExtendToTheme(rootDir, configContent, colorsObj) {
        const configPath = path.join(rootDir, this.config.tailwindConfig);
