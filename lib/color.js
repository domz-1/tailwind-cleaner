...
class ColorReplacer {
    ...
    async writeV4Theme(rootDir, newColors, colorsToUpdate) {
        ...
        // First replace any renamed existing colors
        if (colorsToUpdate && colorsToUpdate.size > 0) {
            for (const [oldName, newName] of colorsToUpdate) {
                const escapedOldName = oldName.replace(/[-\/\\^$*+?.()|[\]{}]/g, '\\$&');
                const regex = new RegExp(`(--color-)${escapedOldName}(\\s*:)`, 'g');
        ...
        try {
            ...
            if (allColorEntries.length > 0) {
                ...
                const { wantRename } = await inquirer.prompt({
                    type: 'confirm',
                    name: 'wantRename',
                    message: chalk.cyan('Do you want to review and rename ANY colors (new or existing)?'),
                    default: false,
                });
                ...
                if (wantRename) {
                    ...
                    for (const [hex, currentName] of allColorEntries) {
                        ...
                        const { newName } = await inquirer.prompt({
                            type: 'input',
                            name: 'newName',
                            message: `${swatch} ${chalk.dim(`#${hex}`)} ${source} ${chalk.dim('Current:')} ${chalk.yellow(currentName)} ${chalk.cyan('New name:')}`,
                            default: currentName,
                            validate: (input) => {
