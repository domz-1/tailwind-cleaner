...
class ColorReplacer {
    ...
    // Convert name to kebab-case CSS-friendly and avoid conflicts
    sanitizeName(name) {
            ...
            newContent = newContent.replace(pattern, (match, prefix, value) => {
                ...
                if (hex) {
                    ...
                    if (colorName) {
                        ...
                        this.replacements++;
                        modified = true;
                        // If this was a Tailwind config match, use the exact prefix structure
                        if (this.tailwindColors.has(hexLower)) {
    ...
    async updateExtendColors(configContent, colorsObj) {
        const configPath = path.join(process.cwd(), CONFIG.tailwindConfig);
        ...
        if (match) {
            ...
            console.log(
                `✅ Updated existing theme.extend.colors with ${
                    Object.keys(colorsObj).length
                } new colors`
            );
            return true;
        }
    ...
    async updateThemeColors(configContent, colorsObj) {
        ...
        if (match) {
            ...
            await fs.writeFile(configPath, newConfigContent, 'utf8');
            console.log(
                `✅ Converted theme.colors to theme.extend.colors and added ${
                    Object.keys(colorsObj).length
                } new colors`
            );
            return true;
    ...
    async addExtendToTheme(configContent, colorsObj) {
        ...
        if (match) {
            ...
            await fs.writeFile(configPath, newConfigContent, 'utf8');
            console.log(
                `✅ Added extend.colors to existing theme with ${
                    Object.keys(colorsObj).length
                } colors`
            );
            return true;
    ...
    async addColorsToExtend(configContent, colorsObj) {
        ...
        if (match) {
            ...
            await fs.writeFile(configPath, newConfigContent, 'utf8');
            console.log(
                `✅ Added colors to existing extend with ${
                    Object.keys(colorsObj).length
                } colors`
            );
            return true;
    ...
    async addCompleteTheme(configPath, configContent, colorsObj) {
        ...
        if (match) {
            ...
            await fs.writeFile(configPath, newConfigContent, 'utf8');
            console.log(
                `✅ Added complete theme structure with ${
                    Object.keys(colorsObj).length
                } colors`
            );
        } else {
            ...
            console.log(
                `✅ Appended theme colors with fallback method (${
                    Object.keys(colorsObj).length
                } colors)`
            );
        }
...
${formattedColors}
  ...
  plugins: [],
    ...
    async run(rootDir = '.') {
        console.log(
            '🚀 Starting Enhanced Color Pizza color replacement with color-2-name fallback...\n'
        );
        try {
...
if (import.meta.url === `file://${process.argv[1]}`) {
    ...
    console.log(
        '🎨 Enhanced Color Pizza Tailwind Replacer with Config Priority and color-2-name Fallback'
    );
    console.log(
        '===============================================================\n'
    );
